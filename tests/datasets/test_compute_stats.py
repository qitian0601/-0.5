...
def test_compute_ee_so3_relative_action_stats_uses_so3_delta_and_absolute_gripper():
    class TinyDataset:
        def __init__(self):
            self.data = {
                OBS_STATE: [
                    np.array([0, 0, 0, 0, 0, 0, 0.01, 1, 0, 0, 0, 0, 0, 0.02], dtype=np.float32),
                    np.array([1, 0, 0, 0, 0, 0, 0.03, 2, 0, 0, 0, 0, 0, 0.04], dtype=np.float32),
                    np.array([2, 0, 0, 0, 0, 0, 0.05, 3, 0, 0, 0, 0, 0, 0.06], dtype=np.float32),
                ],
                ACTION: [
                    np.array([0.1, 0, 0, 0, 0, 0.1, 0.11, 1.2, 0, 0, 0, 0, -0.2, 0.12], dtype=np.float32),
                    np.array([1.1, 0, 0, 0, 0, 0.1, 0.13, 2.2, 0, 0, 0, 0, -0.2, 0.14], dtype=np.float32),
                    np.array([2.1, 0, 0, 0, 0, 0.1, 0.15, 3.2, 0, 0, 0, 0, -0.2, 0.16], dtype=np.float32),
                ],
                "episode_index": [0, 0, 0],
            }
    ...
    features = {
        OBS_STATE: {"shape": [14]},
        ACTION: {"shape": [14], "names": [f"dim_{idx}" for idx in range(14)]},
    }

    stats = compute_ee_so3_relative_action_stats(TinyDataset(), features, chunk_size=2, num_workers=0)

    np.testing.assert_allclose(stats["mean"][:7], [0.6, 0, 0, 0, 0, 0.1, 0.13], atol=1e-5)
    np.testing.assert_allclose(stats["mean"][7:], [0.7, 0, 0, 0, 0, -0.2, 0.14], atol=1e-5)
