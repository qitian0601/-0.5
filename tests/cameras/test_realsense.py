...
def test_connect_cleans_up_when_warmup_read_fails(monkeypatch):
    config = RealSenseCameraConfig(serial_number_or_name="042", warmup_s=1)
    camera = RealSenseCamera(config)
    ...
    monkeypatch.setattr(camera_realsense.rs, "pipeline", lambda: fake_pipeline)
    monkeypatch.setattr(camera, "_configure_capture_settings", lambda: None)
    monkeypatch.setattr(camera, "_start_read_thread", lambda: None)
    monkeypatch.setattr(camera, "async_read", lambda timeout_ms=200: (_ for _ in ()).throw(TimeoutError("boom")))

    with pytest.raises(TimeoutError):
        camera.connect()

    assert fake_pipeline.stop_called
