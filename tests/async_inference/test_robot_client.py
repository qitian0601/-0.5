...
def test_ready_to_send_observation_handles_zero_action_chunk_size(robot_client):
    """Empty action chunks must not make the observation loop divide by zero."""

    robot_client.action_chunk_size = 0
    robot_client.action_queue = Queue()
...
def test_robot_client_uses_overridable_robot_factory():
    from lerobot.async_inference.configs import RobotClientConfig
    from lerobot.async_inference.robot_client import RobotClient
    from tests.mocks.mock_robot import MockRobot, MockRobotConfig

    class CustomRobotClient(RobotClient):
        def _make_robot(self, robot_config):
            self.factory_config = robot_config
    ...
    client = CustomRobotClient(cfg)

    assert client.factory_config is cfg.robot
