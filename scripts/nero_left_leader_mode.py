#!/usr/bin/env python3
"""Minimal left-arm Nero leader-mode test.

This script is intentionally left-arm only:
connect left Nero on nero_left, enable it, call robot.set_leader_mode(),
...
def enable_or_timeout(robot) -> None:
    ...
    print(f"Enabling {ARM_NAME} Nero on {CHANNEL}...")
    ...
    while time.time() < deadline:
        ...
        if enabled:
            return
    raise TimeoutError(f"Timed out enabling {ARM_NAME} Nero on {CHANNEL}.")
...
def main() -> None:
    ...
    print(f"Connecting {ARM_NAME} Nero on {CHANNEL} with firmware={FIRMWARE}...")
    robot.connect()
    drag_teach_started = False
    try:
