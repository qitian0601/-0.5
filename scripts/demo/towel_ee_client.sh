...
# Nero 双臂 EE-local-SE3 异步推理 client。
# 请先在另一个终端启动 run_nero_ee_infer_server.sh。
...
# 可通过环境变量覆盖，不需要改脚本文件。
POLICY_PATH="${NERO_EE_POLICY_PATH:-/home/chenglong/workplace/nero_teleop_ws/lerobot/outputs/train/pi05_fold_towel_ee_bryce301best/pi05_fold_towel_ee_local_se3_rel_8gpu/checkpoints/016000/pretrained_model_rtc}"
...
args=(
  ...
  # policy 类型。
  --policy_type=pi05
  # 新版 EE-local-SE3 PI05 checkpoint，默认 020000。
