...
# 请先在另一个终端启动 run_nero_pickplace_ee_infer_server.sh。
...
# 可通过环境变量覆盖，不需要改脚本文件。
POLICY_PATH="${NERO_PICKPLACE_EE_POLICY_PATH:-/home/chenglong/workplace/nero_teleop_ws/lerobot/outputs/train/pickplace_ee/checkpoints/016000/pretrained_model_rtc}"
RIGHT_CAN="${NERO_RIGHT_CAN:-nero_right}"
LEFT_CAN="${NERO_LEFT_CAN:-nero_left}"
DRY_RUN="${NERO_DRY_RUN:-false}"
TASK="${NERO_PICKPLACE_EE_TASK:-First place the yellow cube into the front box with the right arm, then place the green cube into the front box with the left arm.}"
...
args=(
  ...
  --right_handeye_camera_to_base_yaml=/home/chenglong/workplace/nero_teleop_ws/data/lerobot/pickplace/handeye_right_arm_tsai.yml
  --left_handeye_camera_to_base_yaml=/home/chenglong/workplace/nero_teleop_ws/data/lerobot/pickplace/handeye_left_arm_tsai.yml
  ...
  # 高速执行器单步最大关节变化。
  --safety.max_executor_step_rad=0.008
  # 高速执行器单步最大夹爪变化。
