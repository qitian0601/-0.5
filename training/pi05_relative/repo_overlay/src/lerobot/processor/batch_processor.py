...
class AddBatchDimensionComplementaryDataStep(ComplementaryDataProcessorStep):
    ...
    def complementary_data(self, complementary_data: dict) -> dict:
        ...
        for mask_key in (ACTION_MASK, OBS_IMAGES_MASK):
            mask = complementary_data.get(mask_key)
            if isinstance(mask, Tensor) and mask.dim() == 1:
                complementary_data[mask_key] = mask.unsqueeze(0)
