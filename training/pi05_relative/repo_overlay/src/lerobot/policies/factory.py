...
def _reconnect_relative_absolute_steps(
    ...
    if relative_step is None:
        ee_relative_step = next(
            (s for s in preprocessor.steps if isinstance(s, EERelativeActionsProcessorStep)), None
        )
        ee_local_se3_relative_step = next(
            (s for s in preprocessor.steps if isinstance(s, EELocalSE3RelativeActionsProcessorStep)), None
        )
        if ee_relative_step is None and ee_local_se3_relative_step is None:
            return
        for step in postprocessor.steps:
            if isinstance(step, EEAbsoluteActionsProcessorStep) and step.relative_step is None:
                step.relative_step = ee_relative_step
            if (
                isinstance(step, EELocalSE3AbsoluteActionsProcessorStep)
                and step.relative_step is None
            ):
                step.relative_step = ee_local_se3_relative_step
        return
    for step in postprocessor.steps:
