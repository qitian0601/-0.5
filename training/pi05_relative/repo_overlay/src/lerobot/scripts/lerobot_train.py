...
def _make_training_sampler(active_cfg: Any, dataset: Any, streaming: bool) -> tuple[bool, EpisodeAwareSampler | None]:
    drop_n_last_frames = getattr(active_cfg, "drop_n_last_frames", None)
    if drop_n_last_frames is None or drop_n_last_frames <= 0:
        return True, None

    if streaming:
        raise ValueError(
            "drop_n_last_frames requires an episode-aware sampler, which is not supported with "
            "dataset.streaming=true. Disable streaming or set drop_n_last_frames=null."
        )
...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    shuffle, sampler = _make_training_sampler(active_cfg, dataset, streaming=cfg.dataset.streaming)
