...
def _compute_ee_so3_relative_chunk_batch(
    ...
    """Vectorised EE/SO(3) relative-action computation for a batch of chunk starts."""
    if len(start_indices) == 0:
        return np.empty((0, all_actions.shape[1]), dtype=np.float32)
    ...
    relative = to_relative_ee_actions(
        torch.from_numpy(chunks),
        torch.from_numpy(states),
    )
    return relative.numpy().astype(np.float32, copy=False).reshape(-1, all_actions.shape[1])


def _compute_ee_local_se3_relative_chunk_batch(
    start_indices: np.ndarray,
    all_actions: np.ndarray,
    all_states: np.ndarray,
    chunk_size: int,
) -> np.ndarray:
    """Vectorised EE-local SE(3) relative-action computation for a batch of chunk starts."""
    if len(start_indices) == 0:
        return np.empty((0, all_actions.shape[1]), dtype=np.float32)
    ...
    relative = to_relative_ee_local_se3_actions(
        torch.from_numpy(chunks),
        torch.from_numpy(states),
    )
    return relative.numpy().astype(np.float32, copy=False).reshape(-1, all_actions.shape[1])
...
def compute_ee_so3_relative_action_stats(
    ...
    """Compute action stats for dual-arm EE/SO(3) relative PI0.5 training.

    Dataset actions are expected to be absolute EE target poses with 14-D layout:
    ``[xyz, rotvec, gripper]`` for right arm, then left arm. For each valid
    action chunk, every future action is transformed relative to the chunk
    start ``observation.state``:

    ``delta_pos = target_pos - current_pos``
    ``delta_rot = log(R_target @ R_current.T)``
    ``gripper = target_gripper``

    The returned distribution matches what ``EERelativeActionsProcessorStep``
    feeds to the policy before action normalization.
    """
    action_dim = features[ACTION]["shape"][0]
    if action_dim != 14:
        raise ValueError(f"EE/SO3 relative action stats require 14-D actions, got {action_dim}")

    logging.info("Loading action/state data for EE/SO3 relative action stats...")
    ...
    if all_actions.shape[-1] != 14 or all_states.shape[-1] != 14:
        raise ValueError(
            "EE/SO3 relative action stats require action and observation.state to both be 14-D; "
            f"got action={all_actions.shape[-1]}, state={all_states.shape[-1]}"
        )
...
def compute_ee_local_se3_relative_action_stats(
    hf_dataset,
    features: dict,
    chunk_size: int,
    num_workers: int = 0,
) -> dict[str, np.ndarray]:
    """Compute action stats for 16-D EE-local SE(3) relative PI0.5 training.

    Dataset actions are expected to be absolute egocentric targets with layout:
    ``[xyz, rotvec, gripper]`` for right arm, then left arm, followed by
    ``[base_x, base_y]``. For each valid action chunk, every future action is
    transformed relative to the chunk start ``observation.state``:

    ``T_delta = inv(T_current) @ T_target`` for each EE pose
    ``base_delta = base_target - base_current`` for the final 2 dims
    ``gripper = target_gripper``

    The returned distribution matches what ``EELocalSE3RelativeActionsProcessorStep``
    feeds to the policy before action normalization.
    """
    action_dim = features[ACTION]["shape"][0]
    if action_dim != 16:
        raise ValueError(f"EE-local SE(3) relative action stats require 16-D actions, got {action_dim}")

    logging.info("Loading action/state data for EE-local SE(3) relative action stats...")
    all_actions = np.array(hf_dataset[ACTION], dtype=np.float32)
    all_states = np.array(hf_dataset[OBS_STATE], dtype=np.float32)
    episode_indices = np.array(hf_dataset["episode_index"])

    if all_actions.shape[-1] != 16 or all_states.shape[-1] != 16:
        raise ValueError(
            "EE-local SE(3) relative action stats require action and observation.state to both be 16-D; "
            f"got action={all_actions.shape[-1]}, state={all_states.shape[-1]}"
        )
    ...
    effective_workers = max(num_workers, 1)
    logging.info(
        f"Computing EE-local SE(3) relative action stats from {len(valid_starts)} chunks "
        f"(chunk_size={chunk_size}, workers={effective_workers})"
    )
    ...
    if num_workers > 1:
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as pool:
            futures = [
                pool.submit(
                    _compute_ee_local_se3_relative_chunk_batch,
                    batch,
                    all_actions,
                    all_states,
                    chunk_size,
        ...
        for batch in batches:
            running_stats.update(
                _compute_ee_local_se3_relative_chunk_batch(batch, all_actions, all_states, chunk_size)
            )

    stats = running_stats.get_statistics()
    total_frames = len(valid_starts) * chunk_size
    logging.info(
        f"EE-local SE(3) relative action stats ({len(valid_starts)} chunks, {total_frames} frames): "
        f"mean={np.abs(stats['mean']).mean():.4f}, std={stats['std'].mean():.4f}, "
        f"q01={stats['q01'].mean():.4f}, q99={stats['q99'].mean():.4f}"
    )
