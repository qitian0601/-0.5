...
from lerobot.utils.constants import ACTION_MASK, OBS_ENV_STATE, OBS_IMAGE, OBS_IMAGES, OBS_IMAGES_MASK, OBS_STATE
...
def test_valid_masks_1d_to_2d():
    """Test that PI0.5 mixed-training masks get a batch dimension."""
    processor = AddBatchDimensionProcessorStep()

    action_mask = torch.ones(14, dtype=torch.bool)
    image_mask = torch.tensor([True, False, False])
    transition = create_transition(
        complementary_data={
            ACTION_MASK: action_mask,
            OBS_IMAGES_MASK: image_mask,
        },
        observation={},
    ...
    assert processed_comp_data[ACTION_MASK].shape == (1, 14)
    assert processed_comp_data[OBS_IMAGES_MASK].shape == (1, 3)
    assert torch.equal(processed_comp_data[ACTION_MASK][0], action_mask)
    assert torch.equal(processed_comp_data[OBS_IMAGES_MASK][0], image_mask)
